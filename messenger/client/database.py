...
class ClientDatabase:
    ...
    def __init__(self, name):
        ...
        # Таблица с доступными пользователями
        users = Table('available_users', self.metadata,
        ...
        # Создаём отображения
        mapper(self.AvailableUsers, users)
    ...
    # Функция добавления доступных пользователей.
    ...
    # Функция возвращает список доступных пользователей
    def get_users(self):
        return [user[0] for user in self.session.query(self.AvailableUsers.username).all()]

    # Функция проверяет наличие пользователя в таблице доступных пользователей
    def check_user(self, user):
        if self.session.query(self.AvailableUsers).filter_by(username=user).count():
            return True
        else:
            return False
...
if __name__ == '__main__':
    ...
    test_db.save_message('test2', 'test1', f'Еще тестовое сообщение от {datetime.datetime.now()}!')
    print(test_db.get_contacts())
    print(test_db.get_users())
    print(test_db.check_user('test1'))
    print(test_db.check_user('test11'))
